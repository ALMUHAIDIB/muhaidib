...
class SalesTargetModel(models.Model):
    ...
    def compute_value_target(self):
        for rec in self:
            if rec.product_id and rec.target:
                pricelist_item_obj = self.env['product.pricelist.item'].search([('product_tmpl_id','=',rec.product_id.product_tmpl_id.id),('pricelist_id.name','=','WHOLE SALE')],limit=1)
    ...
    def unique_attendance_code(self):
        if self.product_id and self.user_id and self.month  and self.search([('product_id', '=', self.product_id.id),('user_id', '=', self.user_id.id),('month', '=', self.month),('id', '!=', self.id)]):
    ...
    def compute_target_achieved(self):
        for rec in self:
            ...
            if rec.user_id and rec.product_id and rec.month:
                invoice_lines_obj = self.env['account.invoice.line'].search([('salesman_id','=',rec.user_id.id),('state','in',('open','in_payment','paid')),('price_subtotal_signed','>',0),('product_id','=',rec.product_id.id)])
                ...
                invoice_lines_return_obj = self.env['account.invoice.line'].search(
                    [('salesman_id', '=', rec.user_id.id), ('state', 'in', ('open','in_payment','paid')), ('price_subtotal_signed', '<', 0),
    ...
    def compute_sold_value_achieved(self):
        for rec in self:
            ...
            returned_value = 0
            if rec.user_id and rec.product_id and rec.month:
                invoice_lines_obj = self.env['account.invoice.line'].search(
                    [('salesman_id', '=', rec.user_id.id),('price_subtotal_signed','>',0),('state', 'in',('open','in_payment','paid')),
                     ('product_id', '=', rec.product_id.id)])
                for line in invoice_lines_obj:
                    if line.create_date.month == int(rec.month):
                        filtered_invoice.append(line)
                ...
                invoice_lines_returned_obj = self.env['account.invoice.line'].search(
                    [('salesman_id', '=', rec.user_id.id),('price_subtotal_signed','<',0),('state', 'in',('open','in_payment','paid')),
                     ('product_id', '=', rec.product_id.id)])
